...
echo 📥 [1/9] 爬取今日重大公告 (mops_crawler.py)...
python backend/mops_crawler.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo ⚙️ [2/9] 解析新發行可轉債公告 (parser.py)...
python backend/parser.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo 📈 [3/9] 抓取個股最新歷史股價 (stock_fetcher.py)...
python backend/stock_fetcher.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo 👥 [4/9] 抓取今日三大法人籌碼資料 (inst_fetcher.py)...
python backend/inst_fetcher.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo 🔄 [5/9] 合併法人籌碼至股價資料庫 (merge_inst_to_prices.py)...
python backend/merge_inst_to_prices.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo 🧹 [6/9] 清理與修正 NaN 無效數值 (clean_nan_prices.py)...
python backend/clean_nan_prices.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo 🔗 [7/9] 串聯可轉債多階段生命週期 (bond_stage_linker.py)...
python backend/bond_stage_linker.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo 📊 [8/9] 重算量化策略回測指標 (analyzer.py)...
python backend/analyzer.py
if %errorlevel% neq 0 (echo ❌ 執行失敗! && pause && exit /b %errorlevel%)

echo 🚇 [9/9] 更新執行中 SOP 軌道路線狀態 (daily_monitor.py)...
